...
Gemini CLI-specific configuration and behavior for this repository.
...
3. `GEMINI.md` (this file) — Gemini CLI glue only.
...
## Gemini CLI Behavior in This Repo
...
Gemini CLI must explain the plan aloud before asking the user to approve it.
...
- Use a subagent for read-heavy exploration (codebase search,
  "where is X handled?") to keep the main context lean when subagents
  are available.
...
- Treat the frozen zones listed in `AGENTS.md` and constitution Article VI
  as read-only even when no tool hook enforces them mechanically. If a task
  appears to require touching them, stop and escalate per Article VI.2.
- Gemini CLI loads `.gemini/settings.json` hooks for agent/session lifecycle and
  design-artifact synchronization. Do not bypass or disable them. They do not
  infer semantic outcomes, so run the recording command required by `AGENTS.md`
  for lessons, decisions, approaches, issues, problems, and trade-offs.
- After context compression in a long session, re-read `constitution.md`
  before continuing if its plan-gate or minimal-diff rules are no longer
  present in context.
...
- Keep Gemini-specific guidance in this file. Put guidance that applies to
  every coding agent in `AGENTS.md` instead, and avoid duplicating it here.
- After context compression in long sessions, re-verify you still hold
  the constitution's plan-gate and minimal-diff rules; if unsure, re-read
  `constitution.md`. Constraints lost to compression are still binding.

## Do Not

- Do not restate or paraphrase AGENTS.md/constitution content here when
  editing this file — link, don't copy (drift risk).
- Do not compress context mid-implementation of a multi-step plan; finish
  the step or checkpoint first.
- Do not commit local Gemini CLI settings or files containing credentials.

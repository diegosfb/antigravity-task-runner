...
HTTP 204 = deleted. HTTP 403 = insufficient permissions. HTTP 404 = issue not found.
**Always confirm deletion with the user before executing. Irreversible.**
...
Both MCP and REST are supported. Use MCP when available.
...
curl -s -u "$JIRA_EMAIL:$JIRA_API_TOKEN" \
  ...
  "${JIRA_BASE_URL}rest/api/3/issueLinkType" | jq '.issueLinkTypes[] | {id, name, inward, outward}'
```

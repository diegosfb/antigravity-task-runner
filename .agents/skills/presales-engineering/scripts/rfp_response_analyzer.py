...
# Illustrative defaults. Supply decision_policy in the input for an
# organization-approved screen; the result remains advisory.
DEFAULT_BID_THRESHOLD = 0.70
DEFAULT_CONDITIONAL_THRESHOLD = 0.50
DEFAULT_MAX_MUST_HAVE_GAPS_FOR_BID = 3
        ...
        if req["coverage_status"] in ("gap", "partial", "planned"):
            ...
            if not mitigation:
                if req["coverage_status"] == "partial":
                    ...
                    mitigation = "Obtain product-owner approval before mentioning roadmap status; describe any verified interim option separately"
                else:
...
def determine_bid_recommendation(
    ...
    conditional_threshold: float = DEFAULT_CONDITIONAL_THRESHOLD,
    max_must_have_gaps: int = DEFAULT_MAX_MUST_HAVE_GAPS_FOR_BID,
    policy_supplied: bool = False,
) -> dict[str, Any]:
    ...
    if coverage_ratio >= bid_threshold and must_have_gaps <= max_must_have_gaps:
        decision = "BID"
        reasons.append(f"Coverage score {overall_coverage:.1f}% exceeds {bid_threshold*100:.0f}% threshold")
        if must_have_gaps > 0:
            reasons.append(f"{must_have_gaps} must-have gap(s) within configured range (max {max_must_have_gaps})")
    elif coverage_ratio >= conditional_threshold or (
        must_have_gaps <= max_must_have_gaps and coverage_ratio >= max(conditional_threshold - 0.1, 0)
    ):
        ...
        if must_have_gaps > max_must_have_gaps:
            reasons.append(f"{must_have_gaps} must-have gaps exceed maximum of {max_must_have_gaps}")
    ...
    return {
        "decision": decision,
        "advisory_only": True,
        "policy_source": "input" if policy_supplied else "illustrative_defaults",
...
def generate_risk_assessment(
    ...
    if planned_count > 3:
        risks.append({
            ...
            "mitigation": "Confirm roadmap statements with the accountable product owner; route any proposed commitment through commercial and legal approval",
        })
...
def analyze_rfp(data: dict[str, Any]) -> dict[str, Any]:
    ...
    bid_recommendation = determine_bid_recommendation(
        ...
        float(policy.get("bid_threshold", DEFAULT_BID_THRESHOLD)),
        float(policy.get("conditional_threshold", DEFAULT_CONDITIONAL_THRESHOLD)),
        int(policy.get("max_must_have_gaps", DEFAULT_MAX_MUST_HAVE_GAPS_FOR_BID)),
        bool(policy),
    )
...
def format_text(result: dict[str, Any]) -> str:
    ...
    lines.append(f"Policy source: {bid['policy_source']} (advisory only)")
    lines.append("-" * 70)

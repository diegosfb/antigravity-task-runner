...
def format_text(results: List[Dict[str, Any]]) -> str:
    ...
    lines: List[str] = []
    lines.append("=" * 72)
    lines.append("RETENTION-RISK INDICATOR REPORT")
    lines.append("=" * 72)
    lines.append("Heuristic output: not a churn probability or prediction; missing signals can understate risk.")
    lines.append("")
...
def format_json(results: List[Dict[str, Any]]) -> str:
    ...
    total = len(results)
    output = {
        "report": "retention_risk_indicator",
        "methodology_warning": "Configurable heuristic; not a churn probability or prediction. Missing inputs can understate risk.",
        "summary": {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Calculate a heuristic retention-risk indicator from supplied behavioral signals."
    )

...
description: >
  Construct and govern an authoritative UI design system: token architecture,
  component contracts, responsive rules, documentation, versioning, and
  developer handoff. Use when creating or deliberately extending the system.
  Do not use for extracting or auditing an existing codebase, or for designing
  individual screens and interaction flows.
...
- Use `ui-design-system` to create or govern reusable tokens, component APIs, variants, responsive rules, lifecycle policy, and implementation handoff.
- Use `design-system` first when the job is to discover, document, or audit what an existing frontend repository already implements. Treat its evidence as input; do not silently replace established tokens.
- Use `ui-designer` for screen layout, visual composition, flows, and interaction behavior. Promote a repeated, approved pattern into this skill only when it should become a reusable system contract.
- A feature-specific mockup is not a design-system change. A new reusable token, component contract, or governance rule is.

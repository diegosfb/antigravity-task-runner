...
- If all 3 pass, send the evidence to the [`product-agent`](../../../agents/product-agent/product-agent.md) for PRD creation.
...
- For V3 (cannibalization), record the post-launch guardrail and success metric in the product-agent's PRD so implementation and test planning can make the instrumentation concrete.
...
- After 60 days post-launch (if shipped), use the [`customer-interview-script`](../../customer-interview-script/SKILL.md) debrief workflow with the V2 concierge customers and feed the qualified evidence back to product-agent.

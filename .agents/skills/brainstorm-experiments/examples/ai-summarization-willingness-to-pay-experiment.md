...
- If the experiment lands in the 4-12% band, run a follow-up pricing experiment and send its evidence to the [`product-agent`](../../../agents/product-agent/product-agent.md) for the PRD's viability and pricing assumptions.
- If shipped, product-agent must capture AI evaluation, guardrail, model-selection, and cost constraints in the PRD before architecture and specification work begins.
- Pre-build assumption mapping for the broader feature should use [`../identify-assumptions/`](../identify-assumptions/).
- A pre-mortem for the production version uses [`pre-mortem`](../../pre-mortem/SKILL.md) to surface Tiger/Paper Tiger/Elephant risks.
- The 5 qualitative follow-up calls use the [`customer-interview-script`](../../customer-interview-script/SKILL.md) debrief workflow and feed qualified findings into the opportunity tree and PRD evidence.
- If killed, preserve the decision and evidence in the experiment register and revisit it only when new evidence materially changes the underlying assumption.

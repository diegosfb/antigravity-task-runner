...
The JQL must include **all issue types** so every work item appears on the board.

```bash
curl -s -u "$JIRA_EMAIL:$JIRA_API_TOKEN" \
  -X POST -H "Accept: application/json" -H "Content-Type: application/json" \
  "${JIRA_BASE_URL}rest/api/3/filter" \
  -d '{"name": "<NAME> Kanban Filter", "jql": "project = <KEY> AND issuetype in standardIssueTypes() ORDER BY created DESC"}'
```

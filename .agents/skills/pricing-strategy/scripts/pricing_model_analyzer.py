    ...
    for i, tier in enumerate(tiers):
        ...
        if i > 0:
            ...
            if prev_price > 0:
                ...
                if multiplier < 1.5:
                    issues.append(f"{tier_info['name']} is only {multiplier:.1f}x the previous tier -- verify that differentiation is meaningful")
                elif multiplier > 5:
                    issues.append(f"{tier_info['name']} is {multiplier:.1f}x the previous tier -- test whether the gap leaves unmet demand")
                else:
                    score += 10
...
def analyze_pricing_signals(data: dict) -> dict:
    ...
    if trial_conversion is not None:
        if trial_conversion > 40:
            diagnostics.append({"signal": "Trial-to-paid > 40%", "diagnosis": "Possible underpricing, strong fit, or cohort bias", "action": "Compare with segment and historical baselines before a controlled price test"})
        elif 15 <= trial_conversion <= 30:
            diagnostics.append({"signal": f"Trial-to-paid {trial_conversion}%", "diagnosis": "No conclusion without a relevant baseline", "action": "Evaluate price, packaging, activation, and unit economics together"})
    ...
    if years_since_change is not None and years_since_change >= 2:
        diagnostics.append({"signal": f"Price unchanged for {years_since_change} years", "diagnosis": "Pricing may be stale", "action": "Review value, costs, contracts, and market evidence before testing an increase"})

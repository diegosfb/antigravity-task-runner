...
def analyze_sensitivity(data: dict) -> dict:
    ...
    recommendations = []
    if opp:
        recommendations.append(f"Van Westendorp Optimal Price Point (OPP): ${opp:.2f} -- a directional survey intersection, not a demand-maximizing forecast")
    if pmc and pme:
        recommendations.append(f"Survey-derived acceptable range: ${pmc:.2f} to ${pme:.2f} -- validate it with segment evidence and behavioral tests")
    if idp:
    ...
    if opp and pmc and pme:
        if pmc <= opp <= pme:
            recommendations.append("OPP falls within the survey-derived range; commercial viability still requires behavioral and unit-economic validation")
        else:

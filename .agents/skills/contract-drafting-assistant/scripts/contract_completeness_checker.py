...
Check whether a structured contract draft contains expected sections for a
given jurisdiction and engagement type. This is a structural completeness
check, not legal review or a compliance determination.

Usage:
    python contract_completeness_checker.py contract.json --jurisdiction us-delaware
    python contract_completeness_checker.py contract.json --jurisdiction eu --json
"""
...
def check_contract(data: dict, jurisdiction: str, contract_type: str) -> dict:
    ...
    completeness_pct = (passed / total_required * 100) if total_required > 0 else 0
...
def format_text(result: dict) -> str:
    ...
    lines = []
    lines.append("=" * 60)
    lines.append(f"CONTRACT COMPLETENESS CHECKER")
    lines.append("=" * 60)

    lines.append(f"\nType: {result['contract_type']}")
    lines.append(f"Jurisdiction: {result['jurisdiction']}")
    lines.append(f"Status: {result['status']}")
    lines.append(f"Structural completeness: {result['completeness_pct']}% ({result['clauses_present']}/{result['clauses_checked']})")
...
def main():
    parser = argparse.ArgumentParser(
        description="Check a structured contract draft for expected populated sections."
    )

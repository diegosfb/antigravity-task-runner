...
For a complete worked session, read
[`examples/b2b-onboarding-redesign-trio-session.md`](examples/b2b-onboarding-redesign-trio-session.md).

## Bundled tools

- Run `scripts/idea_scorer.py` when deterministic weighted ranking is useful.
- Run `scripts/opportunity_tree_builder.py` to generate an Opportunity Solution
  Tree from structured outcomes, opportunities, and solutions.
- Run `scripts/scamper_generator.py` when the trio needs structured SCAMPER
  prompts to broaden the option set.

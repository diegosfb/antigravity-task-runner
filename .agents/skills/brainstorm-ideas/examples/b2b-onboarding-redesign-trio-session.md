...
- The 14 customer interviews underpinning the opportunity tree should be synthesized using the [`customer-interview-script`](../../customer-interview-script/SKILL.md) debrief and evidence-quality workflow.
- Each bet that ships becomes evidence for the [`product-agent`](../../../agents/product-agent/product-agent.md), which owns the PRD and its success metrics.
- The activation metric and monitoring expectation belong in the product-agent's PRD goals and success metrics; the downstream implementation and test plans define the instrumentation.

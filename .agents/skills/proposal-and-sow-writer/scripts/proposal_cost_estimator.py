...
Generate a planning price with phase breakdown, payment schedule,
and target gross-margin analysis. Supports multiple currencies.
...
def estimate_cost(hourly_rate: float, hours: float, num_phases: int,
    ...
    total_cost = base_cost / (1 - margin / 100.0)
    margin_amount = total_cost - base_cost
    ...
    risk_buffer_price = round(risk_buffer_cost / (1 - margin / 100.0), 2)
    ...
    return {
        "inputs": {
            ...
            "target_gross_margin_pct": margin,
            "currency": currency,
        },
        "cost_summary": {
        ...
        "risk_buffer": {
            ...
            "buffer_price": risk_buffer_price,
            "total_with_buffer": round(total_cost + risk_buffer_price, 2),
            "note": "Optional 15% contingency, priced at the target gross margin; confirm before quoting",
        },
...
def _generate_recommendations(total_cost: float, hours: float, phases: int) -> list:
    ...
    if total_cost > 50000:
        recs.append({
            "priority": "HIGH",
            "recommendation": "Large engagement -- confirm commercial approval, milestone gates, and invoicing assumptions.",
        })
    if total_cost > 10000:
        recs.append({
            "priority": "MEDIUM",
...
def format_text(result: dict) -> str:
    ...
    lines.append(f"Gross Margin ({result['inputs']['target_gross_margin_pct']}%):    {cur} {cs['margin_amount']:>10,.2f}")
...
def main():
    ...
    parser.add_argument("--margin", type=float, default=20,
                        help="Target gross margin percentage, 0 <= margin < 100 (default: 20)")
    ...
    if args.margin < 0 or args.margin >= 100:
        print("Error: --margin must be at least 0 and less than 100.", file=sys.stderr)
        sys.exit(1)

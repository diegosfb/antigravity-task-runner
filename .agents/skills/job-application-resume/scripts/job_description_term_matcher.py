#!/usr/bin/env python3
"""
Job Description Term Matcher — measure lexical overlap between a resume and a
job description. This is not an ATS simulator or candidate-fit score.

Usage:
    python job_description_term_matcher.py resume.txt jd.txt
    python job_description_term_matcher.py resume.txt jd.txt --json
"""
...
def score(resume_text: str, jd_text: str) -> dict:
    ...
    if not jd_signal:
        return {
            "score": 0,
            "metric": "weighted_term_coverage_pct",
            "kept": [],
            "missing": [],
            "resume_only": [],
...
def render_human(result: dict) -> str:
    lines = []
    lines.append(f"Weighted term coverage: {result['score']}%")
    if "note" in result:
    ...
    lines.append("")
    lines.append("Note: lexical overlap only; this does not measure candidate fit, ATS ranking, or interview probability.")
    return "\n".join(lines)


def main() -> int:
    parser = argparse.ArgumentParser(description="Measure resume/job-description term overlap.")

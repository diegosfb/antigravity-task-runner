...
and surfaces items marked as red flags. It does not determine professional
clearance or whether a transaction should proceed.
...
DEFAULT_STORE = os.path.join(os.getcwd(), "ma-due-diligence.json")
...
def generate_report(data):
    ...
    deal_status = "NO MARKED RED FLAGS"
    if len(all_red_flags) > 0:
        p1_flags = [f for f in all_red_flags if f["priority"] == 1]
        if p1_flags:
            deal_status = "ESCALATE - Priority 1 items marked as red flags"
        else:
            deal_status = "REVIEW - Items marked as red flags"
    ...
    return {
        ...
        "items": items,
        "disclaimer": "Tracker status only; not legal, financial, tax, security, regulatory, people, or transaction clearance."
    }

...
All allocations, discounts, timelines, and scenario labels are illustrative
assumptions. Outputs are arithmetic scenarios, not probabilities, forecasts,
valuations, or market benchmarks.
...
def calculate_synergies(revenue_synergies, cost_synergies, confidence, timeline_months, deal_value=None):
    ...
    scenarios = {
        "optimistic": {
            ...
            "label": "user estimate without confidence discount"
        },
        "base": {
            "revenue": revenue_weighted,
            "cost": cost_weighted,
            "total": total_weighted,
            "label": "illustrative confidence-weighted case"
        },
        "conservative": {
            ...
            "label": "illustrative downside case"
        }
    }
    ...
    return {
        ...
        "warnings": [
            "Illustrative arithmetic only; not a probability, forecast, valuation, fairness opinion, or market benchmark.",
...
def print_human(result):
    ...
    for name, sc in result["scenarios"].items():
        print(f"  {name.upper():<15s} ${sc['total']:>10,}  ({sc['label']})")

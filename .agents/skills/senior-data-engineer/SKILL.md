...
with DAG('daily_etl', schedule_interval='0 5 * * *', catchup=False, tags=['etl']) as dag:
    extract = BashOperator(task_id='extract', bash_command='run-project-extract --date {{ ds }}')
...
| `architecture-designer` | Architecture reviews for lakehouse vs warehouse decisions |

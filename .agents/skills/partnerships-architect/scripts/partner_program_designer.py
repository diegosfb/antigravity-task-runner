...
def design_program(org: dict[str, Any]) -> ProgramDesign:
    if org.get("baseline_assumptions_acknowledged") is not True:
        raise ValueError("set baseline_assumptions_acknowledged: true only after reviewing the illustrative tier model")
    required = ["name", "stage", "target_active_partners", "icp_segment", "partners_per_manager", "year1_costs"]
    missing = [key for key in required if key not in org]
    if missing:
        raise ValueError("explicit program inputs required: " + ", ".join(missing))
    name = org.get("name", "Unnamed Org")
    stage = org["stage"]
    target_partners = int(org["target_active_partners"])
    icp = str(org["icp_segment"]).lower()
    partners_per_manager = float(org["partners_per_manager"])
    if target_partners < 1 or partners_per_manager <= 0:
        raise ValueError("target_active_partners and partners_per_manager must be positive")
    ...
    cm_count = max(1, math.ceil(target_partners / partners_per_manager))
    rationale.append(f"Channel manager scenario: {cm_count}, using the supplied ratio of 1 per {partners_per_manager:g} active partners")
    ...
    costs = org["year1_costs"] or {}
    required_costs = ["manager_loaded_cost", "portal", "enablement", "legal_review", "travel_events", "pilot_mdf"]
    missing_costs = [key for key in required_costs if key not in costs]
    if missing_costs:
        raise ValueError("explicit year1_costs required: " + ", ".join(missing_costs))
    cm_cost = cm_count * float(costs["manager_loaded_cost"])
    portal_dev = float(costs["portal"])
    enablement_content = float(costs["enablement"])
    legal = float(costs["legal_review"])
    travel_events = float(costs["travel_events"])
    pilot_mdf = float(costs["pilot_mdf"])
...
def render_markdown(d: ProgramDesign) -> str:
    out = [f"# Partner Program Baseline: {d.org_name}", "", "> Illustrative scenario only. Tier economics and maturity assumptions require organization-specific validation and approval.", ""]

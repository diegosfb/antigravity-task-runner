...
def model(spec: dict[str, Any]) -> ROIAnalysis:
    ...
    required_revenue = ["revenue_year1", "revenue_year2", "revenue_year3"]
    missing_revenue = [key for key in required_revenue if key not in spec]
    required_costs = [
        "pm_headcount_y1", "eng_integration_y1", "marketing_y1", "enablement_y1", "travel_events_y1", "exec_time_y1",
        "pm_headcount_y2", "eng_ongoing_y2", "marketing_y2", "enablement_y2", "travel_events_y2", "exec_time_y2",
        "pm_headcount_y3", "eng_ongoing_y3", "marketing_y3", "enablement_y3", "travel_events_y3", "exec_time_y3",
    ]
    costs = spec.get("costs") or {}
    missing_costs = [key for key in required_costs if key not in costs]
    if missing_revenue or missing_costs:
        missing = missing_revenue + [f"costs.{key}" for key in missing_costs]
        raise ValueError("explicit scenario inputs required: " + ", ".join(missing))
    ...
    base_costs_y1 = {
        "partnership_manager_headcount": float(costs["pm_headcount_y1"]),
        "engineering_integration_onetime": float(costs["eng_integration_y1"]),
        "marketing_co_launch": float(costs["marketing_y1"]),
        "partner_enablement": float(costs["enablement_y1"]),
        "travel_events": float(costs["travel_events_y1"]),
        "exec_time_allocation": float(costs["exec_time_y1"]),
    }
    costs_y2 = {
        "partnership_manager_headcount": float(costs["pm_headcount_y2"]),
        "engineering_ongoing": float(costs["eng_ongoing_y2"]),
        "marketing": float(costs["marketing_y2"]),
        "partner_enablement": float(costs["enablement_y2"]),
        "travel_events": float(costs["travel_events_y2"]),
        "exec_time_allocation": float(costs["exec_time_y2"]),
    }
    costs_y3 = {
        "partnership_manager_headcount": float(costs["pm_headcount_y3"]),
        "engineering_ongoing": float(costs["eng_ongoing_y3"]),
        "marketing": float(costs["marketing_y3"]),
        "partner_enablement": float(costs["enablement_y3"]),
        "travel_events": float(costs["travel_events_y3"]),
        "exec_time_allocation": float(costs["exec_time_y3"]),
    }

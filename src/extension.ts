...
type NodeKind = "category" | "agent" | "workflow" | "config" | "folder" | "separator" | "action";
        ...
        platform === "codex"
          ...
          : platform === "ollama"
            ...
            : platform === "antigravity"
              ? "Antigravity Agent"
              : "OpenClaude";
      const platformItem = new NodeItem(
        { kind: "config", label: `Agentic Platform: ${platformLabel}` },
        vscode.TreeItemCollapsibleState.None
      );
...
function missingRootItem(): NodeItem {
  const item = new NodeItem(
    { kind: "category", label: "Set antigravity.rootPath" },
    vscode.TreeItemCollapsibleState.None
  );
...
function getScriptPath(rootPath: string, scriptName: string): string {
  const repoRoot = getRepoRoot(rootPath);
...
function getAgenticPlatform(): "openclaude" | "codex" | "ollama" | "antigravity" {
  const platform = vscode.workspace.getConfiguration("antigravity").get<string>("agenticPlatform");
  if (platform === "codex") return "codex";
  if (platform === "ollama") return "ollama";
  if (platform === "antigravity") return "antigravity";
  return "openclaude";
}

async function selectAgenticPlatform(): Promise<void> {
  ...
  if (!selection || selection.value === current) return;
...
function getOpenClaudeExecutable(): string {
  const antigravityConfig = vscode.workspace.getConfiguration("antigravity");
  const override = antigravityConfig.get<string>("openClaudePath");
  if (override) return override;
  const openClaudeConfig = vscode.workspace.getConfiguration("openclaude");
  return openClaudeConfig.get<string>("executablePath") || "openclaude";
}
...
function getCodexExecutable(): string {
  return vscode.workspace.getConfiguration("antigravity").get<string>("codexPath") || "codex";
}

function getCodexArgsTemplate(): string {
  return (
    vscode.workspace.getConfiguration("antigravity").get<string>("codexArgs") ||
    'exec "@{agent} on this project" --skip-git-repo-check'
  );
}

function getOllamaExecutable(): string {
  return vscode.workspace.getConfiguration("antigravity").get<string>("ollamaPath") || "ollama";
}

function getOllamaArgsTemplate(): string {
  return vscode.workspace.getConfiguration("antigravity").get<string>("ollamaArgs") || 'run "{agent}"';
}
...
async function runRepoScript(
  ...
  const scriptPath =
    (homeScript && fs.existsSync(homeScript) ? homeScript : undefined) ||
    (fs.existsSync(repoScript) ? repoScript : undefined);

  if (!scriptPath) {
    void vscode.window.showErrorMessage(
      `Script not found: ${homeScript ? " ~/.antigravity/scripts/" : ""}scripts/${scriptName}.sh`
    );
    return;
  }
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("antigravity.selectPlatform", async () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("antigravity.autocommitRevert", async () => {
      ...
      if (!scriptPath) {
        void vscode.window.showErrorMessage(
          `Script not found: ${path.join(repoRoot, "scripts", "autocommit_revert")}`
        );
        return;
      }

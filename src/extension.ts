...
import {
  BUSINESS_ANALYST_COMMAND,
  buildBusinessAnalystCommand,
  getMissingBusinessAnalystFields,
  renderBusinessAnalystHtml,
  sanitizeBusinessAnalystFormValues,
  type BusinessAnalystFormValues
...
import {
  ...
  buildPlanExecutionCommand,
  getMissingPlanExecutionFields,
  renderPlanExecutionHtml,
  sanitizePlanExecutionFormValues,
  type PlanExecutionFormValues
} from "./planExecution";
import {
  SOLUTION_ARCHITECT_COMMAND,
  buildSolutionArchitectCommand,
  getMissingSolutionArchitectFields,
  renderSolutionArchitectHtml,
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const launchClaudeInit = async (
    ...
    await runClaudeInitAndUpdateInPersistentTerminal(repoRoot, prompt);
  ...
  const launchBusinessAnalystInNewTerminal = (
    values: BusinessAnalystFormValues
  ): void => {
    const terminal = vscode.window.createTerminal({
  ...
  const launchSolutionArchitectInNewTerminal = (
    values: SolutionArchitectFormValues
  ): void => {
    const terminal = vscode.window.createTerminal({
  ...
  const launchPlanExecutionInNewTerminal = (
  ...
  const getProjectScopedStateKey = (prefix: string): string => {
    ...
    const rootPath = getRootPath();
    const projectRoot = rootPath ? getRepoRoot(rootPath) : workspaceRoot;
...
<html lang="en">
  ...
  const renderFeatureEstimatorHtml = (
    webview: vscode.Webview,
...
<html lang="en">
  ...
  const showFeatureEstimatorDialog = async (
    repoRoot: string
  ): Promise<FeatureEstimatorDialogResult | undefined> =>
    new Promise((resolve) => {
      const savedProjectKey = getSavedJiraProjectKey(repoRoot);
      const panel = vscode.window.createWebviewPanel(
        "featureEstimator",
      ...
      let availableIssues: JiraIssueSummary[] = [];
      ...
      let settled = false;
      const resolveOnce = (value: FeatureEstimatorDialogResult | undefined) => {
        if (settled) return;
        settled = true;
        resolve(value);
      };
      ...
      panel.webview.onDidReceiveMessage(
        async (message) => {
          ...
          const featureDetails =
            typeof payload.featureDetails === "string" ? payload.featureDetails.trim() : "";
          if (!featureDetails) {
            void panel.webview.postMessage({
              type: "featureEstimatorError",
    ...
    <form id="jira-project-setup-form">
      ...
      <div class="section">
        ...
        <ol class="instructions">
          ...
          <li>It will select "${JIRA_COMPANY_MANAGED_WORKFLOW_SCHEME}".</li>
...
<html lang="en">
  ...
  const writeAgentPromptFile = (filePrefix: string, prompt: string): string => {
    const sanitizedPrefix =
      filePrefix.replace(/[^a-z0-9-]+/gi, "-").replace(/^-+|-+$/g, "") || "agent-prompt";
  ...
  const launchAgentForJiraItem = async (
    ...
    runInPersistentTerminal(
      issue ? `${agentLabel}: ${issue.key}` : `${agentLabel}: ${backlogItem?.fileName ?? "backlog-item"}`,
      lines,
      {
...
<html lang="en">
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("antigravity.runClaudeAgent", async (agentName: string) => {
      ...
      runInPersistentTerminal(`Agent: ${agentName}`, [
        `cd ${quoteShellArg(repoRoot)}`,
        runString
      ], {
        iconPath: new vscode.ThemeIcon("robot", CLAUDE_ACTION_COLOR),
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("antigravity.openOpenClaudeTerminal", async () => {
      ...
      const repoRoot = getRepoRoot(rootPath);
      runInPersistentTerminal(getAgentTerminalName(), [`cd ${quoteShellArg(repoRoot)}`, "openclaude"], {
        iconPath: new vscode.ThemeIcon("robot", CLAUDE_ACTION_COLOR),
        color: CLAUDE_ACTION_COLOR
      });
    })
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("antigravity.setupWorkspace", async () => {
      showOutputChannel();
      logAlways("[Setup Workspace] Command triggered");
      const workspaceRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
      if (!workspaceRoot) {
        logAlways("[Setup Workspace] ERROR: No workspace folder is open");
    ...
    vscode.commands.registerCommand("antigravity.addJiraItem", async () => {
      ...
      try {
        ...
        const hint = /log in|not authorized|cannot create/i.test(message)
          ...
          : "";
        void vscode.window.showErrorMessage(
          `Failed to load Jira item types for project ${projectKey}: ${message}${hint}`
        );
        return;
      }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("antigravity.assignJiraItemToAgent", async () => {
      ...
      if (!projectKey) {
        void vscode.window.showErrorMessage(
          "Assign Backlog Item to Agent is disabled because JIRA_PROJECT_KEY is not set for this repository."
        );
        provider.refresh();
        return;
      }
      ...
      if (selection.action === "grillMe") {
        ...
        logAlways(
          `[assignJiraItemToAgentGrillMe] launching Agentic Harness for ${issue?.key ?? selectedBacklogItem?.fileName ?? "local backlog item"} with selected command`
        );
        runInPersistentTerminal(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("antigravity.cloudArchitectReview", async () => {
      ...
      const repoRoot = getRepoRoot(rootPath);
      const cloudInfrastructureSignals = detectCloudInfrastructureSignals(repoRoot, 3);
      if (cloudInfrastructureSignals.length === 0) {
        void vscode.window.showInformationMessage(
          "Cloud Architect Review stays visible for this project, but it is disabled until cloud infrastructure signals are detected."
        );
        return;
      }
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        logAlways(`[cloudArchitectReview] ERROR preparing skill: ${message}`);
      ...
      logAlways(
        `[cloudArchitectReview] launching Agentic Harness with signals: ${cloudInfrastructureSignals.join(", ")}`
      );
      runInPersistentTerminal(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("antigravity.pullRemoteAndMerge", async () => {
      ...
      try {
        ...
        if (currentBranch === "main") {
          void vscode.window.showInformationMessage(
            "Pull Remote and merge is only available when you are on a branch other than main."
          );
          return;
        }
        if (currentBranch === "detached HEAD") {
          void vscode.window.showErrorMessage(
            "Pull Remote and merge is unavailable while Git is in a detached HEAD state."
          );
          return;
        }
        ...
        if (!fs.existsSync(scriptPath)) {
          void vscode.window.showErrorMessage(
            "Pull Remote and merge script not found in the extension package."
          );
          return;
        }
        ...
        void vscode.window.showInformationMessage(
          `Pull Remote and merge started on '${currentBranch}'. The workflow will update local main, merge it into '${currentBranch}', run tests, and push the branch.`
        );
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("antigravity.createRepoTag", async () => {
      ...
      runInPersistentTerminal("Antigravity", [
        `cd ${quoteShellArg(repoRoot)}`,
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(PRODUCT_DESIGNER_COMMAND, async () => {
      ...
      const initialValues = sanitizeProductDesignerFormValues(savedValues, workspaceRoot);
      const panel = vscode.window.createWebviewPanel(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(SOLUTION_ARCHITECT_COMMAND, async () => {
      const openWorkspaceRoot = getWorkspaceRoot();
      const rootPath = getRootPath();
      const workspaceRoot = rootPath
        ? resolveProjectWorkspaceRoot(getRepoRoot(rootPath))
        : resolveProjectWorkspaceRoot(openWorkspaceRoot);
      const savedValues = context.workspaceState.get<Partial<SolutionArchitectFormValues>>(
        getProjectScopedStateKey("solutionArchitectForm")
      );
      const initialValues = sanitizeSolutionArchitectFormValues(savedValues, workspaceRoot);
      const panel = vscode.window.createWebviewPanel(
        "antigravitySolutionArchitect",
        "Solution Architect",
        vscode.ViewColumn.Active,
        {
          enableScripts: true,
      ...
      panel.webview.html = renderSolutionArchitectHtml(panel.webview, initialValues);
      panel.webview.onDidReceiveMessage(
        async (message) => {
          if (!message) return;
          ...
          if (message.type === "saveSolutionArchitectDraft") {
            const draftValues = sanitizeSolutionArchitectFormValues(
              message.payload as Partial<SolutionArchitectFormValues> | undefined,
              workspaceRoot
            );
            await context.workspaceState.update(
              getProjectScopedStateKey("solutionArchitectForm"),
          ...
          const values = sanitizeSolutionArchitectFormValues(
            message.payload as Partial<SolutionArchitectFormValues> | undefined,
            workspaceRoot
          );
          const missingFields = getMissingSolutionArchitectFields(values);
          if (missingFields.length > 0) {
            void panel.webview.postMessage({

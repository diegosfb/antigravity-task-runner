...
<html lang="en">
  ...
  const buildJiraAgentPrompt = (
    ...
    const jiraAccessInstructions =
      agentLabel === "Codex"
        ? ` Jira access for this environment is available through the configured Jira MCP server. Use Jira MCP tools for all Jira actions in this task instead of shelling out to the Atlassian CLI. All Jira comments and transitions for this Codex flow must be performed while authenticated to Jira MCP as diegosfbf@gmail.com, because Jira will attribute the actions to the currently authenticated Atlassian account. Before making Jira changes, verify the Jira MCP session is using diegosfbf@gmail.com. If Jira MCP is not authenticated yet or is authenticated as a different Atlassian user, run \`codex mcp login jira\` and sign in as diegosfbf@gmail.com, then continue with the MCP-backed Jira actions. Inspect Jira item ${issueKey}, add each assumption as a Jira comment line beginning with "assuming ", add a final Jira comment beginning with "AGENT SOLUTION:", and transition Jira item ${issueKey} to In Review by using Jira MCP actions.`
        : "";

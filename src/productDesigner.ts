...
export const PRODUCT_DESIGNER_COMMAND = "antigravity.openProductDesigner";

export type ProductDesignerFormValues = {
  agentHarness: string;
  agentModel: string;
  agentIntelligence: string;
  projectDescriptionDir: string;
  meetingsRecordingsFolder: string;
  workspace: string;
...
export function sanitizeProductDesignerFormValues(
  values: Partial<ProductDesignerFormValues> | undefined,
  workspaceRoot?: string
): ProductDesignerFormValues {
...
export function getMissingProductDesignerFields(
...
export function buildProductDesignerCommand(values: ProductDesignerFormValues): string {
  const parts = [
    quoteShellArg(values.agentScriptPath),
    "--project-description-dir",
    quoteShellArg(values.projectDescriptionDir),
    "--workspace",
  ...
  if (values.meetingsRecordingsFolder) {
    parts.push("--meetings-recordings-folder", quoteShellArg(values.meetingsRecordingsFolder));
  }
  if (values.projectConfluence) {
    parts.push("--project-confluence", quoteShellArg(values.projectConfluence));
  }
...
export function renderProductDesignerHtml(
  webview: vscode.Webview,
  initialValues: ProductDesignerFormValues
): string {

...
export const BUSINESS_ANALYST_COMMAND = "antigravity.openBusinessAnalyst";

export type BusinessAnalystFormValues = {
  agentHarness: string;
  agentModel: string;
...
function getDefaultProjectFolders(workspace: string): Pick<BusinessAnalystFormValues, "specsDir" | "backlogDir"> {
  return {
...
function normalizeLegacyProjectFolder(
  value: string,
  workspace: string,
  folderName: "specs" | "backlog"
): string {
...
export function getDefaultBusinessAnalystValues(
  workspaceRoot?: string
): BusinessAnalystFormValues {
...
export function sanitizeBusinessAnalystFormValues(
  values: Partial<BusinessAnalystFormValues> | undefined,
  workspaceRoot?: string
): BusinessAnalystFormValues {
...
export function getMissingBusinessAnalystFields(
...
export function buildBusinessAnalystCommand(values: BusinessAnalystFormValues): string {
  const parts = [
    quoteShellArg(values.agentScriptPath),
    ...(values.specsDir ? ["--specs-dir", quoteShellArg(values.specsDir)] : []),
...
export function renderBusinessAnalystHtml(
  webview: vscode.Webview,
  initialValues: BusinessAnalystFormValues,

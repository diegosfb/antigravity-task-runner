...
export const DEVELOPER_COMMAND = "antigravity.openDeveloper";
...
function getDefaultSourceOutputFolder(workspace: string): string {
...
export function getDefaultDeveloperValues(workspaceRoot?: string): DeveloperFormValues {
...
export function sanitizeDeveloperFormValues(
  values: Partial<DeveloperFormValues> | undefined,
...
export function getMissingDeveloperFields(values: DeveloperFormValues): string[] {
  ...
  if (!values.executionPlanFile) missing.push("Project Execution Plan file");
...
export function buildDeveloperCommand(values: DeveloperFormValues): string {
  const parts = [
...
export function renderDeveloperHtml(

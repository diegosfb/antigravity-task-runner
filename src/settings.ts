...
export const DEFAULT_PROJECT_STRUCTURE_AND_AGENTS_REPOSITORY = "https://github.com/diegosfb/antigravity-task-runner";
...
export async function readClaudeAnthropicBaseUrl(
  repoRoot: string
  ...
  if (fs.existsSync(projectSettings)) {
    try {
      ...
      if (typeof env["ANTHROPIC_BASE_URL"] === "string") return env["ANTHROPIC_BASE_URL"];
    } catch {
      // fall through
    }
  }
...
export function getToolRunCommand(
  ...
  return typeof data[name] === "string" ? (data[name] as string).trim() : undefined;
}
...
export async function loadOpenRouterConfig(): Promise<Record<string, unknown> | null> {
...
export async function loadClaudeSettings(): Promise<ClaudeSettings | null> {
  ...
  try {
    ...
    const data = JSON.parse(raw) as Record<string, unknown>;
    const envRaw =
      typeof data["env"] === "object" && data["env"] ? (data["env"] as Record<string, unknown>) : {};
    return {
      env: {
...
interface SettingsField {
  ...
  placeholder: string;
  value: string;
  type?: "text" | "checkbox" | "command-list";
  checked?: boolean;
  options?: string[];
  optionsKey?: string;
}

interface SdlcSettingsField {
  path: string;
  label: string;
  description: string;
  note?: string;
...
interface SdlcSettingsGroup {
  title: string;
  description: string;
  fields: SdlcSettingsField[];
}
...
export function getBuildCommand(): string {
  const config = vscode.workspace.getConfiguration("antigravity");
  return (config.get<string>("buildCommand") || "").trim();
}
...
function getBundledSdlcSettingsPath(): string {
  return path.resolve(__dirname, "..", "ADLC_workflow_settings.json");
}

export function getSdlcSettingsPath(repoRoot: string): string {
...
function loadSdlcSettingsData(repoRoot: string | undefined): Record<string, unknown> {
  if (repoRoot) {
...
function getNestedValue(data: Record<string, unknown>, dottedPath: string): unknown {
...
function setNestedValue(data: Record<string, unknown>, dottedPath: string, value: unknown): void {
...
function optionList(options: Record<string, string> | undefined): Array<{ value: string; description: string }> {
  if (!options) return [];
  return Object.entries(options).map(([value, description]) => ({ value, description }));
}

function booleanOptionDescription(
...
function stringValue(data: Record<string, unknown>, dottedPath: string, fallback = ""): string {
  const value = getNestedValue(data, dottedPath);
  return typeof value === "string" ? value : fallback;
}

function booleanValue(data: Record<string, unknown>, dottedPath: string, fallback: boolean): boolean {
  const value = getNestedValue(data, dottedPath);
  return typeof value === "boolean" ? value : fallback;
}

function selectOptions(data: Record<string, unknown>, dottedPath: string): Array<{ value: string; description: string }> {
  const value = getNestedValue(data, dottedPath);
  return value && typeof value === "object" && !Array.isArray(value)
    ? optionList(value as Record<string, string>)
    : [];
}

function getSdlcSettingsGroups(repoRoot: string | undefined): SdlcSettingsGroup[] {
  const data = loadSdlcSettingsData(repoRoot);
  const gateField = (key: string, label: string): SdlcSettingsField => ({
  ...
  const mandatoryGateField = (key: string, label: string): SdlcSettingsField => ({
  ...
  return [
    {
      ...
      fields: [
        ...
        },
        {
          ...
          note: booleanOptionDescription(
            data,
            "spoken_plan._options",
            "interrupted_by_next_command_true",
            "interrupted_by_next_command_false"
          ),
          type: "checkbox",
...
export function writeSdlcWorkflowSettings(repoRoot: string, values: Record<string, unknown>): void {
  const data = loadSdlcSettingsData(repoRoot);
  for (const [dottedPath, rawValue] of Object.entries(values)) {
    if (!SDLC_WRITABLE_PATHS.has(dottedPath)) continue;
...
function getExtensionSettingsFields(): SettingsField[] {
  ...
  return [
    ...
    },
    {
      ...
      value: config.get<string>("customAgenticPlatformAddons") || ""
    },
    {
...
export function renderAntigravitySettingsHtml(webview: vscode.Webview): string {
...
export function renderAgenticSetupHtml(
  webview: vscode.Webview,
  initialValues: { claudeGithub?: string; geminiGithub?: string; codexGithub?: string }
): string {
  const nonce = getNonce();
  const csp = `default-src 'none'; img-src ${webview.cspSource} data:; style-src ${webview.cspSource} 'unsafe-inline'; script-src 'nonce-${nonce}';`;
...
export function renderClaudeModelConfigHtml(
  webview: vscode.Webview,
  config: Record<string, unknown>,
  ...
  for (const router of routers) {
    ...
    if (settings) routerSettings[router] = settings;
  }
  const payload = {
    routers,
    effortLevels,

...
exports.buildAssignBacklogItemToAgentFeatureDetails = buildAssignBacklogItemToAgentFeatureDetails;
exports.buildAssignBacklogItemToAgentPrompt = buildAssignBacklogItemToAgentPrompt;
exports.renderAssignBacklogItemToAgentHtml = renderAssignBacklogItemToAgentHtml;
...
function buildAssignBacklogItemToAgentFeatureDetails(issue, backlogItem) {
    const sections = [];
    if (issue) {
    ...
    if (backlogItem) {
        sections.push(`Local backlog item ${backlogItem.displayName} (${backlogItem.fileName}) at ${backlogItem.filePath}`);
...
function buildAssignBacklogItemToAgentPrompt(issue, agentLabel, jiraEmail = "", backlogItem) {
    ...
    const backlogInstructions = backlogItem
        ...
        : "";
    const jiraAccessInstructions = issue && agentLabel === "Codex" && trimmedJiraEmail
...
function renderAssignBacklogItemToAgentHtml(webview, issues, options) {
    const nonce = (0, settings_1.getNonce)();
    const csp = `default-src 'none'; style-src ${webview.cspSource} 'unsafe-inline'; script-src 'nonce-${nonce}';`;
    const selectedIssueKey = options.selectedIssueKey && issues.some((issue) => issue.key === options.selectedIssueKey)
        ? options.selectedIssueKey
        : "";
    const selectedBacklogItemPath = options.selectedBacklogItemPath &&
    ...
    const backlogStatusMessage = options.backlogStatusMessage ||
...
//# sourceMappingURL=assignBacklogItemToAgent.js.map
...
exports.GITHUB_BUILTIN_SECRETS = void 0;
exports.scanWorkflowFiles = scanWorkflowFiles;
exports.loadWorkflowFiles = loadWorkflowFiles;
exports.parseGitHubOwnerRepo = parseGitHubOwnerRepo;
exports.runGh = runGh;
exports.spawnGh = spawnGh;
exports.setGhSecret = setGhSecret;
exports.setGhVariable = setGhVariable;
exports.getGitRemoteUrl = getGitRemoteUrl;
exports.computeDelta = computeDelta;
exports.getSecretHint = getSecretHint;
exports.buildEnvCommentLines = buildEnvCommentLines;
exports.writeEnvDocumentation = writeEnvDocumentation;
exports.runSecretsAudit = runSecretsAudit;
const fs = require("fs");
const path = require("path");
const child_process_1 = require("child_process");
const vscode = require("vscode");
const utils_1 = require("./utils");
exports.GITHUB_BUILTIN_SECRETS = new Set([
...
function ensureEnv(map, env) {
    if (!map[env])
        map[env] = { secrets: [], variables: [] };
}
function addUnique(arr, value) {
    if (!arr.includes(value))
        arr.push(value);
}
function scanWorkflowFiles(files) {
    const result = {};
    for (const file of files) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            for (const m of secretMatches) {
                ...
                if (exports.GITHUB_BUILTIN_SECRETS.has(name))
                    continue;
                ensureEnv(result, currentEnv);
...
function loadWorkflowFiles(repoRoot) {
...
function runGh(args, cwd) {
    return (0, child_process_1.execSync)(`gh ${args}`, { cwd, encoding: "utf8" }).trim();
}
function spawnGh(args, cwd) {
    const result = (0, child_process_1.spawnSync)("gh", args, { cwd, encoding: "utf8" });
    if (result.status !== 0)
        throw new Error(result.error?.message || result.stderr?.toString() || "gh command failed");
    return result.stdout.trim();
}
function setGhSecret(name, value, env, cwd) {
    const args = ["secret", "set", name];
    if (env)
        args.push("--env", env);
    const result = (0, child_process_1.spawnSync)("gh", args, { cwd, input: value, encoding: "utf8" });
    if (result.status !== 0)
        throw new Error(result.error?.message || result.stderr?.toString() || "gh secret set failed");
}
function setGhVariable(name, value, env, cwd) {
    const args = ["variable", "set", name, "--body", value];
    if (env)
        args.push("--env", env);
    const result = (0, child_process_1.spawnSync)("gh", args, { cwd, encoding: "utf8" });
    if (result.status !== 0)
        throw new Error(result.error?.message || result.stderr?.toString() || "gh variable set failed");
}
function getGitRemoteUrl(repoRoot) {
    return (0, child_process_1.execSync)("git remote get-url origin", { cwd: repoRoot, encoding: "utf8" }).trim();
}
function computeDelta(required, existing) {
    const delta = {};
...
function buildEnvCommentLines(required) {
    const lines = ["# === GitHub Secrets & Variables (auto-audited) ==="];
    for (const [env, { secrets, variables }] of Object.entries(required)) {
        for (const s of secrets)
            lines.push(`# GITHUB_SECRET[${env}]: ${s}`);
        for (const v of variables)
            lines.push(`# GITHUB_VARIABLE[${env}]: ${v}`);
    }
    return lines;
}
function writeEnvDocumentation(envPath, required, appendEnvCommentFn) {
    const lines = buildEnvCommentLines(required);
    for (const line of lines) {
        appendEnvCommentFn(envPath, line);
    }
}
async function runSecretsAudit(repoRoot) {
    // Pre-flight checks
    try {
        runGh("--version", repoRoot);
    }
    catch {
    ...
    let environments = [];
    try {
        const envResp = JSON.parse(runGh(`api repos/${owner}/${repo}/environments`, repoRoot));
    ...
    for (const env of environments) {
        ...
        const variables = [];
        try {
            const s = JSON.parse(spawnGh(["secret", "list", "--env", env, "--json", "name"], repoRoot));
            secrets.push(...s.map((x) => x.name));
        }
        catch { /* env may have no secrets */ }
        try {
            const v = JSON.parse(spawnGh(["variable", "list", "--env", env, "--json", "name"], repoRoot));
    ...
    const repoVars = [];
    try {
        const s = JSON.parse(runGh("secret list --json name", repoRoot));
        repoSecrets.push(...s.map((x) => x.name));
    }
    catch { /* ignore */ }
    try {
        const v = JSON.parse(runGh("variable list --json name", repoRoot));
    ...
    const missingItems = Object.entries(delta).flatMap(([env, { secrets, variables }]) => [
        ...secrets.map((s) => ({ env, name: s, type: "secret" })),
        ...variables.map((v) => ({ env, name: v, type: "variable" })),
    ]);
    // Write .env documentation regardless
    const envPath = path.join(repoRoot, ".env");
    writeEnvDocumentation(envPath, required, utils_1.appendEnvComment);
    ...
    const proceed = await vscode.window.showInformationMessage(`Found ${missingItems.length} missing item(s) across environments: ${envNames}. Proceed to set them?`, "Set them", "Skip (document only)");
    if (!proceed || proceed === "Skip (document only)")
        return;
    ...
    if (skipped.length > 0) {
        void vscode.window.showWarningMessage(`✅ Set ${setCount} of ${missingItems.length} items. Skipped: ${skipped.join(", ")}. Run again to retry.`);
    }
    else {
        void vscode.window.showInformationMessage("✅ All secrets and variables are configured.");
    }
}
//# sourceMappingURL=secrets-audit.js.map
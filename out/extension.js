...
exports.buildProjectStructureZipUrl = buildProjectStructureZipUrl;
...
function buildProjectStructureZipUrl(repositoryUrl) {
...
function activate(context) {
    ...
    const execFileText = (command, args, cwd) => new Promise((resolve, reject) => {
        (0, child_process_1.execFile)(command, args, { cwd, maxBuffer: 20 * 1024 * 1024 }, (error, stdout, stderr) => {
            if (error) {
    ...
    const extractProjectStructureEntries = async (zipPath, destination, overwrite, entries) => {
        if (entries.length === 0)
            return;
        await execFileText("unzip", [overwrite ? "-o" : "-n", zipPath, ...entries, "-d", destination]);
    };
    const promptProjectStructureOverwrite = async (relativePath) => {
        const choice = await vscode.window.showWarningMessage(`Project structure file already exists: ${relativePath}`, { modal: true }, "All", "None", "Yes, this file only", "No, not this file");
        if (choice === "All") {
            const confirmation = await vscode.window.showWarningMessage("Overwrite all existing project structure files?", { modal: true }, "Overwrite All", "Cancel");
            return confirmation === "Overwrite All" ? "all" : undefined;
        }
        if (choice === "None")
            return "none";
        if (choice === "Yes, this file only")
            return "yes-this-file";
        if (choice === "No, not this file")
            return "no-this-file";
        return undefined;
    };
    const deployProjectStructureZip = async (repoRoot) => {
        const config = vscode.workspace.getConfiguration("antigravity");
        const source = (config.get("projectStructureAndAgentsRepository") ||
            settings_1.DEFAULT_PROJECT_STRUCTURE_AND_AGENTS_REPOSITORY).trim();
        if (!source) {
        ...
        (0, logger_1.logAlways)(`[Setup Workspace] source: ${source}`);
        (0, logger_1.logAlways)(`[Setup Workspace] zipUrl: ${zipUrl}`);
        (0, logger_1.logAlways)(`[Setup Workspace] workspaceDir: ${workspaceDir}`);
        await fs.promises.mkdir(tmpDir, { recursive: true });
        await fs.promises.mkdir(workspaceDir, { recursive: true });
        await (0, scripts_1.downloadFile)(zipUrl, zipPath);
...
<html lang="en">
  ...
  <body>
    ...
    const showAssignBacklogItemToAgentDialog = async (projectKey, issues, backlogItems, backlogStatusMessage) => new Promise((resolve) => {
        const initialAgentCommand = (0, settings_1.getAgenticHarnessExecutionCommand)();
        const agentCommandOptions = getAssignableAgentCommandOptions();
        const panel = vscode.window.createWebviewPanel("assignBacklogItemToAgent", "Assign Backlog Item to Agent", vscode.ViewColumn.Active, { enableScripts: true });
  ...
  <body>
    ...
    context.subscriptions.push(vscode.commands.registerCommand("antigravity.openClaudeTerminal", async () => {
        ...
        try {
            const rootPath = (0, utils_1.getRootPath)();
            if (!rootPath) {
                (0, logger_1.log)(`[openClaudeTerminal] ERROR: project root not available`);
                void vscode.window.showErrorMessage("TaskRunner needs an open workspace folder to determine the project root.");
                return;
            }
            const repoRoot = (0, utils_1.getRepoRoot)(rootPath);
    ...
    context.subscriptions.push(vscode.commands.registerCommand("antigravity.openAgentMonitorClaudeTerminal", async () => {
        ...
        try {
            const rootPath = (0, utils_1.getRootPath)();
            if (!rootPath) {
                (0, logger_1.log)(`[openAgentMonitorClaudeTerminal] ERROR: project root not available`);
                void vscode.window.showErrorMessage("TaskRunner needs an open workspace folder to determine the project root.");
                return;
            }
            const repoRoot = (0, utils_1.getRepoRoot)(rootPath);
    ...
    context.subscriptions.push(vscode.commands.registerCommand("antigravity.openAgentMonitorCodexTerminal", async () => {
        ...
        try {
            const rootPath = (0, utils_1.getRootPath)();
            if (!rootPath) {
                (0, logger_1.log)(`[openAgentMonitorCodexTerminal] ERROR: project root not available`);
                void vscode.window.showErrorMessage("TaskRunner needs an open workspace folder to determine the project root.");
                return;
            }
            const repoRoot = (0, utils_1.getRepoRoot)(rootPath);
    ...
    context.subscriptions.push(vscode.commands.registerCommand("antigravity.openAgentMonitorOpenCodeTerminal", async () => {
        ...
        try {
            const rootPath = (0, utils_1.getRootPath)();
            if (!rootPath) {
                (0, logger_1.log)(`[openAgentMonitorOpenCodeTerminal] ERROR: project root not available`);
                void vscode.window.showErrorMessage("TaskRunner needs an open workspace folder to determine the project root.");
                return;
            }
            const repoRoot = (0, utils_1.getRepoRoot)(rootPath);
    ...
    context.subscriptions.push(vscode.commands.registerCommand("antigravity.openOllamaCodexTerminal", async () => {
        ...
        try {
            const rootPath = (0, utils_1.getRootPath)();
            if (!rootPath) {
                (0, logger_1.log)(`[openOllamaCodexTerminal] ERROR: project root not available`);
                void vscode.window.showErrorMessage("TaskRunner needs an open workspace folder to determine the project root.");
                return;
            }
            const repoRoot = (0, utils_1.getRepoRoot)(rootPath);
    ...
    context.subscriptions.push(vscode.commands.registerCommand("antigravity.openCodexTerminal", async () => {
        ...
        try {
            const rootPath = (0, utils_1.getRootPath)();
            if (!rootPath) {
                (0, logger_1.log)(`[openCodexTerminal] ERROR: project root not available`);
                void vscode.window.showErrorMessage("TaskRunner needs an open workspace folder to determine the project root.");
                return;
            }
            const repoRoot = (0, utils_1.getRepoRoot)(rootPath);
    ...
    context.subscriptions.push(vscode.commands.registerCommand("antigravity.openOpencodeTerminal", async () => {
        ...
        try {
            const rootPath = (0, utils_1.getRootPath)();
            if (!rootPath) {
                (0, logger_1.log)(`[openOpencodeTerminal] ERROR: project root not available`);
                void vscode.window.showErrorMessage("TaskRunner needs an open workspace folder to determine the project root.");
                return;
            }
            const repoRoot = (0, utils_1.getRepoRoot)(rootPath);
    ...
    context.subscriptions.push(vscode.commands.registerCommand("antigravity.setupWorkspace", async () => {
        ...
        try {
            ...
            (0, logger_1.logAlways)(`[Setup Workspace] Extracted project structure into ${workspaceDir}`);
            void vscode.window.showInformationMessage(`Setup Workspace extracted project structure files into ${workspaceDir}.`);
        }
        catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            (0, logger_1.logAlways)(`[Setup Workspace] ERROR deploying project structure: ${message}`);
            void vscode.window.showErrorMessage(`Setup Workspace failed: ${message}`);
        }
    }));
    ...
    context.subscriptions.push(vscode.commands.registerCommand("antigravity.initRepository", async () => {
        ...
        const rootPath = (0, utils_1.getRootPath)();
        if (!rootPath) {
            (0, logger_1.logAlways)(`[initRepository] ERROR: project root not available`);
            void vscode.window.showErrorMessage("TaskRunner needs an open workspace folder to determine the project root.");
            return;
        }
        const repoRoot = (0, utils_1.getRepoRoot)(rootPath);
    ...
    context.subscriptions.push(vscode.commands.registerCommand("antigravity.initRepositoryConfigUpdate", async () => {
        ...
        const rootPath = (0, utils_1.getRootPath)();
        if (!rootPath) {
            (0, logger_1.logAlways)(`[initRepositoryConfigUpdate] ERROR: project root not available`);
            void vscode.window.showErrorMessage("TaskRunner needs an open workspace folder to determine the project root.");
            return;
        }
        const repoRoot = (0, utils_1.getRepoRoot)(rootPath);
    ...
    context.subscriptions.push(vscode.commands.registerCommand("antigravity.commitChanges", async () => {
        ...
        try {
            const rootPath = (0, utils_1.getRootPath)();
            if (!rootPath) {
                (0, logger_1.logAlways)("[commitChanges] ERROR: project root not available");
                void vscode.window.showErrorMessage("TaskRunner needs an open workspace folder to determine the project root.");
                return;
            }
    ...
    context.subscriptions.push(vscode.commands.registerCommand("antigravity.createClaudeMd", async () => {
        ...
        try {
            ...
            if (!repoRoot) {
                (0, logger_1.log)(`[createClaudeMd] ERROR: no workspace folder`);
                void vscode.window.showErrorMessage("No workspace folder is open.");
                return;
            }
    ...
    context.subscriptions.push(vscode.commands.registerCommand("antigravity.createAgentMd", async () => {
        ...
        try {
            ...
            const repoRoot = rootPath ? (0, utils_1.getRepoRoot)(rootPath) : vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
            if (!repoRoot) {
                (0, logger_1.log)(`[createAgentMd] ERROR: no workspace folder`);
                void vscode.window.showErrorMessage("No workspace folder is open.");
                return;
            }
    ...
    const runAgenticLibScript = async (scriptName) => {
        const rootPath = (0, utils_1.getRootPath)();
        if (!rootPath) {
            void vscode.window.showErrorMessage("TaskRunner needs an open workspace folder to determine the project root.");
            return;
    ...
    context.subscriptions.push(vscode.commands.registerCommand("antigravity.deployAgenticLibSdlc", async () => {
        await runAgenticLibScript("deploy-sdlc-lib.sh");
    }));
    context.subscriptions.push(vscode.commands.registerCommand("antigravity.deployAgenticLibSdlcExtended", async () => {
        await runAgenticLibScript("deploy-extended-sdlc-lib.sh");
    }));
    context.subscriptions.push(vscode.commands.registerCommand("antigravity.deployAgenticLibProfessionalServices", async () => {
        await runAgenticLibScript("deploy-professional-services-lib.sh");
    }));
    context.subscriptions.push(vscode.commands.registerCommand("antigravity.deployAgenticLibTechAdvisory", async () => {
        await runAgenticLibScript("deploy-technical-advisor-lib.sh");
    }));
    context.subscriptions.push(vscode.commands.registerCommand("antigravity.cleanDeployedLibs", async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("antigravity.addBacklogItem", async () => {
        const rootPath = (0, utils_1.getRootPath)();
        if (!rootPath) {
            void vscode.window.showErrorMessage("TaskRunner needs an open workspace folder to determine the project root.");
            return;
        ...
        if (projectKey) {
            ...
            if (addBacklogCredentials) {
                try {
                    issueTypes = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: "Loading item types", cancellable: false }, async () => (0, jira_1.getJiraIssueTypes)(addBacklogCredentials, projectKey));
                }
                catch {
                    issueTypes = [];
                }
        ...
        if (backlogItem.action === "grillMe") {
            try {
                const copiedSkillPaths = await (0, grillMe_1.copyGrillMeSkill)(extensionRoot, repoRoot, resourceProvider);
                (0, logger_1.logAlways)(`[addBacklogItemGrillMe] skill locations ready: ${copiedSkillPaths.length > 0 ? copiedSkillPaths.join(", ") : "already present"}`);
                provider.refresh();
            }
            catch (error) {
                const message = error instanceof Error ? error.message : String(error);
            ...
            const featureDetails = (0, grillMe_1.buildJiraDraftFeatureDetails)(displayProject, backlogItem.issueType, backlogItem.summary, backlogItem.description);
            const prompt = (0, grillMe_1.buildFeatureGrillMePrompt)(featureDetails);
            const promptFilePath = writeAgentPromptFile("add-backlog-item-grill-me", prompt);
            const commandLine = (0, terminal_1.buildAgenticHarnessFileCommand)(repoRoot, promptFilePath, "prompt");
            (0, terminal_1.runInPersistentTerminal)("Add Backlog Item Grill Me", [`cd ${(0, utils_1.quoteShellArg)(repoRoot)}`, commandLine], { iconPath: FEATURE_ESTIMATOR_ICON_PATH, color: FEATURE_ESTIMATOR_ACTION_COLOR });
            void vscode.window.showInformationMessage(`Opened Grill Me for the ${backlogItem.issueType} draft.`);
            return;
        }
        // Write local backlog file
        const backlogDir = path.join(repoRoot, "docs", "backlog");
        const fileContent = (0, backlogItem_1.buildBacklogItemTemplate)({
            issueType: backlogItem.issueType,
            summary: backlogItem.summary,
            description: backlogItem.description
        });
        const backlogFilePath = (0, backlogItem_1.resolveBacklogItemFilePath)(backlogDir, backlogItem.issueType, backlogItem.summary);
        if (backlogFilePath) {
            try {
        ...
        if (addBacklogCredentials && projectKey) {
            try {
                const createdIssue = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: "Creating Jira item", cancellable: false }, async () => (0, jira_1.createJiraIssue)(addBacklogCredentials, {
                    projectKey,
                    issueTypeName: backlogItem.issueType,
                    summary: backlogItem.summary,
                    description: backlogItem.description
                }));
                void vscode.window.showInformationMessage(`Created ${backlogItem.issueType} in Jira (${createdIssue.key}) and saved to docs/backlog.`);
            }
            catch (error) {
                const message = error instanceof Error ? error.message : String(error);
                void vscode.window.showWarningMessage(`Saved to docs/backlog but failed to create Jira item: ${message}`);
            }
        }
        else {
            void vscode.window.showInformationMessage(`Created ${backlogItem.issueType} "${backlogItem.summary}" in docs/backlog.`);
        }
    }));
    ...
    context.subscriptions.push(vscode.commands.registerCommand("antigravity.takeBacklogItemAssign", async () => {
        const rootPath = (0, utils_1.getRootPath)();
        if (!rootPath) {
            void vscode.window.showErrorMessage("TaskRunner needs an open workspace folder to determine the project root.");
            return;
            ...
            if (takeCredentials) {
                try {
                    jiraItems = await vscode.window.withProgress({
                        ...
                        title: `Loading available Jira items from ${projectKey.trim().toUpperCase()}`,
                        cancellable: false
                    }, async () => (0, jira_1.searchOpenUnassignedTodoJiraIssuesForAssignment)(takeCredentials, projectKey));
                }
                catch {
                    jiraItems = [];
                }
        ...
        if (pickItems.length === 0) {
            void vscode.window.showInformationMessage("No available backlog items found. Add items to docs/backlog or configure a Jira project.");
            return;
        }
        ...
        if (selection.source === "jira") {
            ...
            const confirm = await vscode.window.showInformationMessage(`Assign ${issue.key} to ${takeCredentials.email} and move to In Progress?`, { modal: true }, "Assign To Me");
            if (confirm !== "Assign To Me")
                return;
            try {
                await vscode.window.withProgress({
                    ...
                    await (0, jira_1.assignJiraIssueToCurrentUser)(takeCredentials, issue.key);
                    await (0, jira_1.transitionJiraIssueToStatus)(takeCredentials, issue.key, "In Progress");
                });
                void vscode.window.showInformationMessage(`Assigned Jira item ${issue.key} to ${takeCredentials.email} and moved it to In Progress.`);
            }
            catch (error) {
                const message = error instanceof Error ? error.message : String(error);
                void vscode.window.showErrorMessage(`Failed to assign Jira item: ${message}`);
            ...
            const confirm = await vscode.window.showInformationMessage(`Take "${item.displayName}" and mark it as In Progress?`, { modal: true }, "Take Item");
            if (confirm !== "Take Item")
                return;
            try {
                ...
                void vscode.window.showInformationMessage(`Took "${item.displayName}" — marked as In Progress, assigned to ${assignedAs}.`);
            }
            catch (error) {
                const message = error instanceof Error ? error.message : String(error);
    ...
    context.subscriptions.push(vscode.commands.registerCommand("antigravity.assignBacklogItemToAgent", async () => {
        const rootPath = (0, utils_1.getRootPath)();
        if (!rootPath) {
            void vscode.window.showErrorMessage("TaskRunner needs an open workspace folder to determine the project root.");
            return;
            ...
            if (backlogCredentials) {
                try {
                    issues = await vscode.window.withProgress({
                        ...
                        cancellable: false
                    }, async () => (0, jira_1.searchOpenUnassignedTodoJiraIssuesForAssignment)(backlogCredentials, projectKey));
                }
                catch {
                    issues = [];
                }
        ...
        const selection = await showAssignBacklogItemToAgentDialog(projectKey, issues, backlogItems, backlogStatusMessage);
        if (!selection)
            return;
        ...
        if (selection.action === "grillMe") {
            try {
                const copiedSkillPaths = await (0, grillMe_1.copyGrillMeSkill)(extensionRoot, repoRoot, resourceProvider);
                (0, logger_1.logAlways)(`[assignBacklogItemToAgentGrillMe] skill locations ready: ${copiedSkillPaths.length > 0 ? copiedSkillPaths.join(", ") : "already present"}`);
                provider.refresh();
            }
            catch (error) {
                const message = error instanceof Error ? error.message : String(error);
                (0, logger_1.logAlways)(`[assignBacklogItemToAgentGrillMe] ERROR preparing skill: ${message}`);
                void vscode.window.showErrorMessage(`Failed to prepare the grill-me skill: ${message}`);
                return;
            }
            const featureDetails = (0, assignBacklogItemToAgent_1.buildAssignBacklogItemToAgentFeatureDetails)(selectedIssue, selectedBacklogItem);
            const grillPrompt = (0, grillMe_1.buildFeatureGrillMePrompt)(featureDetails);
            const grillPromptFilePath = writeAgentPromptFile("assign-backlog-item-grill-me", grillPrompt);
            const grillCommandLine = (0, agenticHarnessCommand_1.buildAgenticHarnessFileCommandForCommand)(selection.agentCommand, repoRoot, grillPromptFilePath, "prompt");
            (0, terminal_1.runInPersistentTerminal)("Assign Backlog Item Grill Me", [`cd ${(0, utils_1.quoteShellArg)(repoRoot)}`, grillCommandLine], { iconPath: FEATURE_ESTIMATOR_ICON_PATH, color: FEATURE_ESTIMATOR_ACTION_COLOR });
            void vscode.window.showInformationMessage("Opened Grill Me for the selected backlog item with the selected agent harness command.");
            return;
        }
        if (selection.useJira && selectedIssue) {
            ...
            try {
                await vscode.window.withProgress({
                    ...
                    await (0, jira_1.updateJiraIssueSummaryAndLabels)(backlogCredentials, selectedIssue.key, updatedSummary, [buildAgentJiraLabel(agentLabel)]);
                    await (0, jira_1.assignJiraIssueToCurrentUser)(backlogCredentials, selectedIssue.key);
                    await (0, jira_1.transitionJiraIssueToStatus)(backlogCredentials, selectedIssue.key, "In Progress");
                });
            }
            catch (error) {
                const message = error instanceof Error ? error.message : String(error);
        ...
        const agentPrompt = (0, assignBacklogItemToAgent_1.buildAssignBacklogItemToAgentPrompt)(selectedIssue, agentLabel, backlogCredentials?.email ?? "", selectedBacklogItem);
        const agentPromptFilePath = writeAgentPromptFile(`assign-backlog-item-to-agent-${agentLabel.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`, agentPrompt);
        const agentCommand = (0, agenticHarnessCommand_1.buildAgenticHarnessFileCommandForCommand)(selection.agentCommand, repoRoot, agentPromptFilePath, "unattended");
        const terminalLines = agentCommand.includes("\n")
            ? [
                `zsh ${(0, utils_1.quoteShellArg)(writeAgentLaunchScript(`antigravity-${agentLabel.toLowerCase().replace(/[^a-z0-9]+/g, "-")}-backlog`, `cd ${(0, utils_1.quoteShellArg)(repoRoot)}\n${agentCommand}`))}`
            ]
            : [`cd ${(0, utils_1.quoteShellArg)(repoRoot)}`, agentCommand];
        ...
        if ((0, settings_1.getUseExternalTerminal)()) {
            ...
            (0, terminal_1.runInPersistentTerminal)(terminalLabel, terminalLines, {
                iconPath: new vscode.ThemeIcon("robot", terminal_1.CLAUDE_ACTION_COLOR),
                color: terminal_1.CLAUDE_ACTION_COLOR
            });
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand("antigravity.approvePullRequest", async () => {
        ...
        if (!workflowFile) {
            void vscode.window.showErrorMessage("Approve pull request workflow not found in the bundled extension files.");
            return;
        }

...
| Item | What it does | How to use it | When it appears |
| --- | --- | --- | --- |
| `Setup Workspace` | Opens a project template picker populated from `Resources/project-templates.json`, then launches the selected Agentic Harness to download the chosen starter project into the configured workspace path using that template's URL and instructions. | Use it before `Workspace Setup` when you want to seed the workspace with a starter project. | Always |
